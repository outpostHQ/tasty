...
/**
 * A map to connect styles with their handlers by their name.
 */
export function setStyleHandlerMap(map: StyleHandlerMap) {
  Object.assign(STYLE_HANDLER_MAP, map);
}

/**
 * Clear the style handler map.
 */
export function clearStyleHandlerMap() {
  Object.keys(STYLE_HANDLER_MAP).forEach(key => {
